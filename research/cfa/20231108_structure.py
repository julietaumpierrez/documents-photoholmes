...
net = FullNet()
# %%
model = "src/photoholmes/models/cfa/weights/pretrained.pt"
...
np_image.shape
# %%
# NONE COMPRESSION TEST
quality = None
img.shape
# %%
block_size = 32
with torch.no_grad():
    out = net(img.unsqueeze(0), block_size)
# %%
out.shape
# %%
res = np.exp(out.numpy())
# %%
res
# %%
res[:, 1] = res[([1, 0, 3, 2], 1)]
...
np_image.shape
# %%
from src.photoholmes.utils.postprocessing.resizing import upscale_mask

# Generate original image coordinates correctly
# Use indexing='ij' for matrix indexing convention suitable for image processing
x = np.arange(confidence.shape[0])
y = np.arange(confidence.shape[1])
orig_confidence_coords = (x, y)

# The target size is the size of the np_image
target_size = np_image.shape[:2]  # This should be (640, 1024)
target_size
# %%

# Use the upscale_mask function to upscale the confidence map to match the np_image size
confidence_upscaled = upscale_mask(orig_confidence_coords, confidence, target_size)
# %%
confidence_upscaled
# confidence_upscaled will now be an upscaled version of the original mask
# %%

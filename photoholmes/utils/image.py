...
def plot_multiple_images(images, titles=None, ncols=4, title:str = None, save_path=None):
    '''D
    '''
    N = len(images)
...
class Data:
    ...
    def from_path(cls, img_folder_path:str, mask_folder_path:str = None):
        ...
        if mask_folder_path is not None:
            return cls([ImFile.from_path(img_folder_path+name, mask_folder_path+cls._mask_name(name)) for name in img_names], 
                       name=name)
        else:
            return cls([ImFile.from_path(img_folder_path+name) for name in img_names], name=name)
    ...
    def show(self, ncols=4, title:str = None, save_path=None):
        ...
        '''
        plot_multiple_images(self.images, self.names, 
                             ncols=ncols, title=title, save_path=save_path)
